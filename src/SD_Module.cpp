...
static File CSV_Create()
{
  char dir_year[32];
  char path[64];
  build_paths_safe(dir_year, sizeof(dir_year), path, sizeof(path), nullptr, nullptr);
  if (!SD.exists(dir_year)) {
    SD.mkdir(dir_year);
  }
                   ...
                   const char* unidade)
{
  ...
  if (!CSV_rotateIfNeeded()) return false;
