...
namespace {
  ...
  float    g_ir_dc      = 0.0f;
  float    g_red_dc     = 0.0f;
  constexpr float kAlphaDC = 0.01f;
  ...
  unsigned long g_refrac_samples   = 30; // ~300 ms
...
void ppg_init(int sample_rate_sps)
{
  ...
  g_refrac_samples = (unsigned long)(0.30f * (float)g_sps);
  ppg_reset();
}
...
void ppg_feedSample(uint32_t red, uint32_t ir)
{
  ...
  // Pico (IR), dedo presente, limiar relativo
...
void ppg_tick_1s()
{
  ...
  g_spo2     = (spo2 < 70.0f) ? 70.0f : (spo2 > 100.0f ? 100.0f : spo2);
  g_has_spo2 = true;
}
